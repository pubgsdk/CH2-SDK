#pragma once

// Chivalry 2 (1.X) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "CinematicFadeOutListenerInterface_structs.hpp"

namespace SDK
{
//---------------------------------------------------------------------------
//Classes
//---------------------------------------------------------------------------

// BlueprintGeneratedClass CinematicFadeOutListenerInterface.CinematicFadeOutListenerInterface_C
// 0x0000 (0x0028 - 0x0028)
class UCinematicFadeOutListenerInterface_C : public UInterface
{
public:

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindObject<UClass>(_xor_("BlueprintGeneratedClass CinematicFadeOutListenerInterface.CinematicFadeOutListenerInterface_C"));
		return ptr;
	}


	void On_Fade_Out_Started();
	void On_Fade_Out_Finished();
};


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
